...
We find an interesting structure when we look at chains of maximal sublattices: $L/pL$ is isomorphic to the vector space $\mathbb{F}_p^2$ over the finite field $\mathbb{F}_p$, hence any lattice has exactly $p+1$ maximal sublattices. We may form a graph in which vertices are equivalence classes of lattices up to scalar multiplication, and two vertices are adjacent if they contain representatives $L$ and $L'$ such that $L'$ is maximal in $L$. Fascinatingly, this graph turns out to be a tree! We call this the *Bruhat-Tits tree*.
